...
public class Main {
	...
	public static void main(String[] args) throws RemoteException {
		...
		try {
			...
			ShowScreen s = new ShowScreen(screenshot, obj, token);
			s.setVisible(true);
			s.start();
			for(;;){
...
implements MouseListener, MouseMotionListener {
	...
	int altReal, largReal, altJan, largJan;
	Dimension size;
	double dif = 0.98;
	
	private static final long serialVersionUID = -1842261777470977698L;

	public ShowScreen(byte image[], RemoteAccess obj, String token)  {
    ...
    public int coordRealAlt(int coordJanela) {
    	return (int) (((altReal* coordJanela)/altJan)) ;
    }
